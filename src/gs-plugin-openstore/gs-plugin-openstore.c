...
static void
gs_plugin_openstore_app_add_icon (GsApp *app,
                          ...
                          gpointer user_data)
{
  ...
  if (g_strcmp0 (signal_name, "DownloadProgressChanged") == 0) {
  ...
  } else if (g_strcmp0 (signal_name, "InstallStatusChanged") == 0) {
    const gchar *package_id = NULL;
                             ...
                             gpointer user_data)
{
  ...
  while ((child = g_variant_iter_next_value (&iter))) {
    ...
    if (package_name != NULL) {
      ...
      gs_plugin_openstore_app_add_icon (app, icon_path, icon_url);
                                       ...
                                       gpointer user_data)
{
   ...
   * OpenStore apps (e.g. flatpaks). Calling UpgradePackages anyway would
  ...
  if (gs_app_list_length (own_apps) == 0) {
    g_debug ("No OpenStore apps in update list, nothing to do");
    self->current_progress_callback = NULL;
    self->current_progress_user_data = NULL;
    g_task_return_boolean (task, TRUE);
    return;
